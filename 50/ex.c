#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct aluno {
  char *nome;
  int nr;
  int nota;
} Aluno;

typedef struct cel {
  Aluno aluno;
  struct cel *prox;
} *Turma;

int adicionaAluno(Turma *t, char *nome, int nr, int nota) {
  Aluno novo_aluno;
  novo_aluno.nome = malloc(sizeof(char) * (strlen(nome) + 1));
  if (novo_aluno.nome == NULL) {
    return 1; 
  }

  strcpy(novo_aluno.nome, nome); 
  novo_aluno.nr = nr;
  novo_aluno.nota = nota;

  struct cel *nova_cel = malloc(sizeof(struct cel)); 
  if (nova_cel == NULL) {
    free(novo_aluno.nome);
    return 1; 
  }

  nova_cel->aluno = novo_aluno; 

  while (*t != NULL && (*t)->aluno.nr < nr) { 
    t = &((*t)->prox);
  }

  nova_cel->prox = *t;
  *t = nova_cel;

  return 0; 
}

void imprimeTurma(Turma t) {
  printf("Turma:\n");
  while (t != NULL) {
    printf("Nome: %s | Número: %d | Nota: %d\n", t->aluno.nome, t->aluno.nr, t->aluno.nota);
    t = t->prox;
  }
}

void libertaTurma(Turma t) {
  Turma temp;
  while (t != NULL) {
    temp = t;
    t = t->prox;
    free(temp->aluno.nome);
    free(temp);
  }
}

int main() {
  Turma turma = NULL;

  adicionaAluno(&turma, "Ana", 2001, 18);
  adicionaAluno(&turma, "Bruno", 1998, 16);
  adicionaAluno(&turma, "Carla", 2003, 15);
  adicionaAluno(&turma, "Daniel", 1995, 19);
  adicionaAluno(&turma, "Eduarda", 2000, 17);

  imprimeTurma(turma);
  libertaTurma(turma);

  return 0;
}